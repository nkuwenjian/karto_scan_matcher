...
namespace KartoScanMatcher 
{
	...
	ScanMatcher* ScanMatcher::Create(kt_double searchSize, 
									...
									kt_double coarseAngleResolution)
	{
		...
		ScanMatcher* pScanMatcher = new ScanMatcher();
		...
		pScanMatcher->m_pFineSearchAngleOffset = fineSearchAngleOffset;
		pScanMatcher->m_pCoarseSearchAngleOffset = coarseSearchAngleOffset;
		pScanMatcher->m_pCoarseAngleResolution = coarseAngleResolution;
	...
	kt_double ScanMatcher::MatchScan(LocalizedRangeScan* pScan, const LocalizedRangeScanVector& rBaseScans, Pose2& rMean,
									Matrix3& rCovariance, kt_bool doRefineMatch)
	{
		///////////////////////////////////////
		// set scan pose to be center of grid

		// 1. get scan position
		Pose2 scanPose = pScan->GetSensorPose();  // 利用码盘提供激光传感器中心的初始位姿估计
		...
		if (pScan->GetNumberOfRangeReadings() == 0)
		{
			...
			rCovariance(2, 2) = 4 * math::Square(m_pCoarseAngleResolution);  // TH*TH
		...
		kt_double bestResponse = CorrelateScan(pScan, scanPose, coarseSearchOffset, coarseSearchResolution,
											m_pCoarseSearchAngleOffset, m_pCoarseAngleResolution,
		...
		if (doRefineMatch)
		{
			...
			bestResponse = CorrelateScan(pScan, rMean, fineSearchOffset, fineSearchResolution,
									0.5 * m_pCoarseAngleResolution, m_pFineSearchAngleOffset,
									rMean, rCovariance, true);
		}
	...
	kt_double ScanMatcher::CorrelateScan(LocalizedRangeScan* pScan, const Pose2& rSearchCenter,
										...
										Pose2& rMean, Matrix3& rCovariance, kt_bool doingFineMatch)
	{
